...
const createBlog = async function (req, res) {
     try {
          ...
          if (tags){
               if(tags.length == 0){
                    return res.status(400).send({ status: false, message: "Array of tags can not be blank" })
               }
          }
          ...
          if (subcategory){
               ...
               if(subcategory.length == 0){
                    return res.status(400).send({ status: false, message: "Blank space not allowed" })
               }
               
               if(subcategory.length == 0){
                    return res.status(400).send({ status: false, message: "Array of subcategory can not be blank" })
               }
          }
